...
| Reusable helpers used across threads | Interpreter with [functional skills](/oss/python/deepagents/skills#use-functional-skills) |
...
<CodeGroup>
```bash pip
pip install -U deepagents langchain-quickjs
```
...
By default, interpreter state also persists across turns in the same thread by snapshotting the eotkinh state after each agent run, and restoring it before the next run.
...
agent = create_deep_agent(
    model="openai:gpt-5.4",
    middleware=[REPLMiddleware(ptc=["task"])],
)
```
...
Functional skills are [skills](/oss/python/deepagents/skills) that expose code modules to an interpreter. When configured with interpreter middleware, the agent can import these modules from code and use them for deterministic helper logic.

Functional skills are useful when the agent needs reusable helpers for structured data workflows, such as sorting, grouping, scoring, parsing, validating, or aggregating data. For setup details, see [Functional skills](/oss/python/deepagents/skills#use-functional-skills).

## Snapshots and time travel

Python `REPLMiddleware` snapshots interpreter state after each agent run and restores it before the next run by default. A snapshot is a serialized copy of the interpreter's in-memory JavaScript state, including globals, variables, functions, and imported modules that exist when the agent finishes running code.

Across conversation turns, the lifecycle is:

1. A turn starts, and `REPLMiddleware` restores the latest interpreter snapshot for the thread.
...
| Functional skill modules | No | Add a `module` entry and configure `skills_backend` or `skillsBackend` |
...
`REPLMiddleware` accepts the following options:
...
| `skills_backend` | `None` | Backend used to resolve functional skill modules. |
