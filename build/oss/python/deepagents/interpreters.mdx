...
Interpreter state persists across `eval` calls. By default, interpreter state also persists across turns in the same thread by snapshotting the QuickJS context after each agent run and restores it before the next run.
...
Functional skills are skills that expose code modules to the interpreter. Regular skills give the agent instructions and context. Functional skills also give the agent importable functions it can call from interpreter code.
...
Use functional skills for code that should be:
