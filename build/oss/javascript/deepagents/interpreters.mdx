...
| Reusable helpers used across threads | Interpreter with [functional skills](/oss/javascript/deepagents/skills#use-functional-skills) |
...
const agent = createDeepAgent({
  model: "openai:gpt-5.4",
  middleware: [createREPLMiddleware({ ptc: ["task"] })],
});
```
...
Functional skills are [skills](/oss/javascript/deepagents/skills) that expose code modules to an interpreter. When configured with interpreter middleware, the agent can import these modules from code and use them for deterministic helper logic.

Functional skills are useful when the agent needs reusable helpers for structured data workflows, such as sorting, grouping, scoring, parsing, validating, or aggregating data. For setup details, see [Functional skills](/oss/javascript/deepagents/skills#use-functional-skills).
...
`createREPLMiddleware` accepts the following options:
...
| `skillsBackend` | omitted | Backend used to resolve functional skill modules. |
