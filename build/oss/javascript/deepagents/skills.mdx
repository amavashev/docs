...
Deep agent skills follow the [Agent Skills specification](https://agentskills.io/specification) and add additional capability for functional skills which makes it possible to provide skills with importable functions that an interpreter can call.
...
- [Use functional skills](#use-functional-skills) when the agent needs reusable, importable helpers available from interpreter code.
...
### Use functional skills

Functional skills are skills that expose code modules to an [interpreter](/oss/javascript/deepagents/interpreters). Regular skills give the agent instructions and context. Functional skills also give the agent importable functions it can call from interpreter code.
...
const agent = createDeepAgent({
  ...
  backend,
  skills: ["/skills/"],
  middleware: [createREPLMiddleware({ skillsBackend: backend })],
});
```
