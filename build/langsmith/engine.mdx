...
<Steps>
  ...
  <Step title="Select a model provider">
    In the bottom bar, click the provider dropdown and select a model provider (for example, **Anthropic**). The LangSmith Engine uses two models from this provider: a heavy model that reasons over traces and writes the agent overview and issues, and a light model that screens trace batches before deeper analysis.
  </Step>
  <Step title="Set API keys">
    Click the key icon <Icon icon="key" /> in the bottom bar to open **Secrets & API keys** and enter the API key for your selected provider. Keys are stored as workspace secrets and shared with other features. The key icon shows a warning indicator when the key is missing. **Start Analyzing** is disabled until a valid key is saved.
  </Step>
...
<Frame caption="Setup dialog">
  <img
    ...
    alt="Setup dialog showing the code repository field, category selections for prioritizing issue types, and the provider dropdown and key icon in the bottom bar"
  />
  <img
