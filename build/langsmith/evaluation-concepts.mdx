...
An _attachment_ is the association between a single evaluator and a single tracing project or dataset. The same evaluator can have many attachments, and configuration like sampling rate, filters, and [spend limits](/langsmith/evaluator-spend) is set per attachment, not per evaluator. View an evaluator's attachments under its **Projects & Datasets** tab.
