...
| Run Count | Number of runs the evaluator has scored over the last 7 days |
| Spend | Total LLM cost for the evaluator across all attached resources over the last 7 days. Sortable. Evaluators that do not call an LLM (for example, code evaluators) show no value. Refer to [Track and limit evaluator spend](/langsmith/evaluator-spend). |
...
- **Resources**: The tracing projects and datasets this evaluator is attached to, with each attachment's [weekly spend and limit](/langsmith/evaluator-spend).
