...
This page covers the core concepts of context engineering in LangSmith: skills, agents, the Context Hub, versioning, and sharing.

## Skills

A **skill** packages a single, reusable capability that an agent can invoke, stored as a versioned repo in the Context Hub. Skills are focused on doing one thing well, and they're designed to be used by agents.
...
An **agent** is an AI system that completes tasks end to end using tools, skills, and subagents. An **agent repo** packages its complete configuration: high-level instructions, the skills and subagents it can use, and the tools it has access to.
...
The Context Hub is the long-term context database for your agents. Think of it as a Git-like home for your agent's context. It is built for versioning, sharing, and continuous improvement.

A store backend is built for runtime state. It holds the information an agent accumulates _while running_: memories, conversation history, user preferences, learned facts, and other data that evolves per session or per user.
...
- Follow the [Context engineering quickstart](/langsmith/context-engineering-quickstart) to create your first skill or agent in the Context Hub.
