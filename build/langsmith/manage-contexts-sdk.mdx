...
content across repos. For example, an agent that delegates to a shared skill. The linked repo is resolved at pull time:
...
| `parent_commit` / `parentCommit`   | `string` (optional)                         | Parent commit hash for optimistic concurrency. Must be 8–64 hex characters when provided. Raises `LangSmithConflictError` if it doesn't match the latest commit. |
