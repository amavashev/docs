...
repos** and **skill repos** in the [Context Hub](/langsmith/context-engineering-concepts)
programmatically: push new commits, pull contexts at runtime, list and
search repos, check existence, and delete repos. This is useful for
automating context updates from a CI/CD pipeline or version-pinning
contexts for production deployments.
...
| `files`       | map of path to `Entry` | The files in the commit, keyed by their path.        |
...
this for reusable capabilities that other agents can depend on. The
parameters are identical to `push_agent`. Refer to the
[Push an agent](#push-an-agent) parameters table.
...
Same shape as `pull_agent`, returning a `SkillContext`. The parameters
are identical. Refer to the [Pull an agent](#pull-an-agent) parameters
table.
...
removes all file repos nested within it.
</Warning>
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/langsmith/manage-contexts-programmatically.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
